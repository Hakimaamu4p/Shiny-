...
module.exports = {
  config: {
    ...
    category: 'ai',
    longDescription: {
      en: 'Chat With Anime Character Sukuna.',
    },
    guide: {
      en: '',
    },
  ...
  onStart: async ({ api, event, args }) => {
    try {
      ...
      const response = await axios.get(`https://king-aryanapis.onrender.com/api/sukuna?prompt=${encodeURIComponent(prompt)}&uid=${uid}`);
  ...
  onReply: async ({ api, event, Reply }) => {
    ...
    try {
      ...
      const response = await axios.get(`https://king-aryanapis.onrender.com/api/sukuna?prompt=${encodeURIComponent(userReply)}&uid=${uid}`);
